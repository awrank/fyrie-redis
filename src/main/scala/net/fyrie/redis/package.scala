...
package object redis {
  ...
  private[redis] class ParseBulk[Result[_]](value: Result[Option[ByteString]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[Option[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulk[Result[_]](value: Result[Option[List[Option[ByteString]]]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[Option[List[Option[A]]]] = f.fmap(value)(_.map(_.map(_.map(Parse(_)))))
  }
  private[redis] class ParseMultiBulkList[Result[_]](value: Result[List[Option[ByteString]]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[List[Option[A]]] = f.fmap(value)(_.map(_.map(Parse(_))))
    def parse[A: Parse, B: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(2).collect {
      case List(a, b) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(3).collect {
      case List(a, b, c) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(4).collect {
      case List(a, b, c, d) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(5).collect {
      case List(a, b, c, d, e) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(6).collect {
      case List(a, b, c, d, e, f) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)), f map (Parse[F](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(7).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(8).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(9).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(10).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(11).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(12).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(13).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(14).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(15).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(16).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(17).collect {
    ...
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse, R: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(18).collect {
      case List(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)), f map (Parse[F](_)), g map (Parse[G](_)), h map (Parse[H](_)), i map (Parse[I](_)), j map (Parse[J](_)), k map (Parse[K](_)), l map (Parse[L](_)), m map (Parse[M](_)), n map (Parse[N](_)), o map (Parse[O](_)), p map (Parse[P](_)), q map (Parse[Q](_)), r map (Parse[R](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse, R: Parse, S: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(19).collect {
      case List(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)), f map (Parse[F](_)), g map (Parse[G](_)), h map (Parse[H](_)), i map (Parse[I](_)), j map (Parse[J](_)), k map (Parse[K](_)), l map (Parse[L](_)), m map (Parse[M](_)), n map (Parse[N](_)), o map (Parse[O](_)), p map (Parse[P](_)), q map (Parse[Q](_)), r map (Parse[R](_)), s map (Parse[S](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse, R: Parse, S: Parse, T: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(20).collect {
      case List(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)), f map (Parse[F](_)), g map (Parse[G](_)), h map (Parse[H](_)), i map (Parse[I](_)), j map (Parse[J](_)), k map (Parse[K](_)), l map (Parse[L](_)), m map (Parse[M](_)), n map (Parse[N](_)), o map (Parse[O](_)), p map (Parse[P](_)), q map (Parse[Q](_)), r map (Parse[R](_)), s map (Parse[S](_)), t map (Parse[T](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse, R: Parse, S: Parse, T: Parse, U: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(21).collect {
      case List(a, b, c, d, e, f, g, h, i, j, k, l, m, n, o, p, q, r, s, t, u) ⇒ (a map (Parse[A](_)), b map (Parse[B](_)), c map (Parse[C](_)), d map (Parse[D](_)), e map (Parse[E](_)), f map (Parse[F](_)), g map (Parse[G](_)), h map (Parse[H](_)), i map (Parse[I](_)), j map (Parse[J](_)), k map (Parse[K](_)), l map (Parse[L](_)), m map (Parse[M](_)), n map (Parse[N](_)), o map (Parse[O](_)), p map (Parse[P](_)), q map (Parse[Q](_)), r map (Parse[R](_)), s map (Parse[S](_)), t map (Parse[T](_)), u map (Parse[U](_)))
    } toList)
    def parse[A: Parse, B: Parse, C: Parse, D: Parse, E: Parse, F: Parse, G: Parse, H: Parse, I: Parse, J: Parse, K: Parse, L: Parse, M: Parse, N: Parse, O: Parse, P: Parse, Q: Parse, R: Parse, S: Parse, T: Parse, U: Parse, V: Parse](implicit executor: ExecutionContext) = f.fmap(value)(_.grouped(22).collect {
  ...
  private[redis] class ParseMultiBulkFlat[Result[_]](value: Result[Option[List[ByteString]]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[Option[List[A]]] = f.fmap(value)(_.map(_.map(Parse(_))))
  }
  private[redis] class ParseMultiBulkFlatList[Result[_]](value: Result[List[ByteString]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[List[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulkSet[Result[_]](value: Result[Set[ByteString]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[Set[A]] = f.fmap(value)(_.map(Parse(_)))
  }
  private[redis] class ParseMultiBulkMap[Result[_]](value: Result[Map[ByteString, ByteString]])(implicit f: ResultFunctor[Result]) {
    def parse[K: Parse, V: Parse](implicit executor: ExecutionContext): Result[Map[K, V]] = f.fmap(value)(_.map(kv ⇒ (Parse[K](kv._1), Parse[V](kv._2))))
  }
  private[redis] class ParseMultiBulkScored[Result[_]](value: Result[List[(ByteString, Double)]])(implicit f: ResultFunctor[Result]) {
    def parse[A: Parse](implicit executor: ExecutionContext): Result[List[(A, Double)]] = f.fmap(value)(_.map(kv ⇒ (Parse(kv._1), kv._2)))
  }
}
