...
class SortedSetSpec extends mutable.Specification with TestClient {
  ...
  "zadd" >> {
    "should add based on proper sorted set semantics" ! client { r: RedisClient ⇒
      add(r)
  ...
  "zrange" >> {
    "should get the proper range" ! client { r: RedisClient ⇒
      add(r)
  ...
  "zrank" >> {
    "should give proper rank" ! client { r: RedisClient ⇒
      add(r)
  ...
  "zremrangebyrank" >> {
    "should remove based on rank range" ! client { r: RedisClient ⇒
      add(r)
  ...
  "zremrangebyscore" >> {
    "should remove based on score range" ! client { r: RedisClient ⇒
      add(r)
  ...
  "zunion" >> {
    "should do a union" ! client { r: RedisClient ⇒
